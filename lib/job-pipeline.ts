...
export async function insertNormalizedJobs(normalized: NormalizedJob[], log: string[]): Promise<number> {
  ...
  // Classification rotates across Groq + Mistral + OpenRouter so no single
  // provider's daily quota gets exhausted. Cap scales with configured providers
  // (3 batches × N providers). Remainder retries naturally next cron run.
  log.push(`Classifying... (${needsClassification.length} candidates)`)
