...
const CLASSIFY_PROVIDERS: ClassifyProvider[] = [
  {
    ...
    call: async (prompt) => {
      ...
      const res = await fetch('https://api.cerebras.ai/v1/chat/completions', {
        ...
        // gpt-oss-120b is a reasoning model — produces chain-of-thought first
        // (~700 tokens) then the JSON answer. Verified working with 1500 tokens.
        body: JSON.stringify({ model: 'gpt-oss-120b', messages: [{ role: 'user', content: prompt }], temperature: 0, max_tokens: 1500 }),
